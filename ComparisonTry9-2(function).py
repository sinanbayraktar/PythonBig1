...
def MakeComparison (kn_neigh):
	...
	neigh = KNeighborsClassifier(n_neighbors=kn_neigh)
	neigh.fit(feature_train, label_train) 
	pred_neigh = neigh.predict(feature_test)
	acc_neigh = accuracy_score(pred_neigh, label_test)
	print ("Accuracy Score for K Neighbours is ", acc_neigh)

	print ("Here is the graph of the comparison for the ", kn_neigh, " k neighbors")
